...
void Hooks::ClientInstanceScreenModel_sendChatMessage(void* _this, TextHolder* text) {
	...
	if (text->getTextLength() > 0) {
		...
		if (*message == cmdMgr->prefix) {
			...
			if (!helpedUser) {
				helpedUser = true;
				g_Data.getClientInstance()->getGuiData()->displayClientMessageF("%sYour Pocket prefix is: \"%s%c%s\"", RED, YELLOW, cmdMgr->prefix, RED);
...
__int64 Hooks::RenderText(__int64 a1, C_MinecraftUIRenderContext* renderCtx) {
...

	{
		...
		if (strcmp(screenName.c_str(), "start_screen") == 0) {
			// Draw BIG epic Pocket watermark
			{
...

			{
				...
				if (shouldRenderWatermark) {
					...
					static std::string name = "Pocket| Public";
#ifdef _DEBUG
					static std::string version = "discord.gg/pocketclient";
#elif defined _BETA
